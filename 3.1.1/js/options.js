/*
 ========================================
 Polychroma TV - Firefox Extension
 ----------------------------------------
...
var config = [];
$.ajax({
    url: browser.runtime.getURL('config.json'),
    async: false,
    dataType: 'json',
...
$(document).ready(function () {
    ...
    if (config["show_extended_options"] === 'yes') {
        ...
        for (var i = 0; i < config["feeds"].length; i++) {
            ...
            var feed_item = '<div class="feed_item">' + config["feeds"][i]["feed_title"] +
                '<span class="notification_check tooltip--top" data-tooltip="' + browser.i18n.getMessage("options_hide_notifications") + '?"><input type="checkbox" class="feed_notify" id="' + i + '_feed_notifications" name="' + i + '_feed_notifications" ' + feed_notifications_checked + '/><label for="' + i + '_feed_notifications"></label></span>' +
                '<span class="show_check tooltip--top" data-tooltip="' + browser.i18n.getMessage("options_hide_feed") + '?"><input type="checkbox" class="feed_show" id="' + i + '_feed_show" data-id="' + i + '" name="' + i + '_feed_show" ' + feed_show_checked + '/><label for="' + i + '_feed_show"></label></span>' +
                '</div>';
        ...
        $(".feeds").append(browser.i18n.getMessage("options_show_notifications") + ': <span class="show_check tooltip--top" data-tooltip="' + browser.i18n.getMessage("options_hide_notifications") + '"><input type="checkbox" id="show_notifications" name="show_notifications" '+ show_notify_checked +'/><label for="show_notifications"></label></span>');

    }


    $('title').text(browser.i18n.getMessage("options_title"));
    $('label[for="change-time"]').text(browser.i18n.getMessage("options_time_update"));
    ...
    var time_update_title = browser.i18n.getMessage("options_time_update_value").split(',');
...
function setBadgeCount(count) {
    count = (count == '0') ? '' : count;
    browser.browserAction.setTitle({title: browser.i18n.getMessage("badge_text") + count});
    browser.browserAction.setBadgeBackgroundColor({color: config["badge_color"]});
