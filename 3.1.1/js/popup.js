...
$(document).ready(function () {
    ...
    browser.browserAction.getBadgeText({}, function (Badge_txt) {
        var mark = $('.btn-clear');
        if (Badge_txt === '') {
            mark.addClass('none');
        } else {
    ...
    $('.btn-clear').attr("title", browser.i18n.getMessage("mark_read"));
    ...
    if (config['settings_button'] == "show") {
        ...
        $('.btn-settings').attr("title", browser.i18n.getMessage("options_title"));
    ...
    if (show_feed_ids.length !== 0) {
        ...
        $(".nofeeds").append('<div><p>' + browser.i18n.getMessage("nofeeds_message") + '</p><button class="btn btn-settings">' + browser.i18n.getMessage("options_title") + '</button></div>');
    ...
    $('.btn-clear').on('click', function () {
        ...
        browser.browserAction.setTitle({title: config['title']});
...
function setDate(date) {
    ...
    var month = browser.i18n.getMessage("month").split(",");

    var today = new Date();
    tm = today.getMonth() + 1;
    td = today.getDate();
    date = ((d == td) && (month[m] == tm)) ? browser.i18n.getMessage("today") : d + ' ' + month[m] + ' ' + y;
...
function getContent(feed_id, feed_url, feed_type, feed_more_button, feed_socials, feed_description, feed_image, feed_image_type) {
    ...
    $.get(feed_url, function (data) {
        ...
        $(data).find(item).each(function () {
            ...
            var more = '';
            more = (feed_more_button == "show") ? '<a href="' + url + '" data-id="' + news_id + '" class="btn btn-more link" target="_blank">' + browser.i18n.getMessage("more") + '</a>' : more;
